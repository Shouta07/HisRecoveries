...
export default function StepsSection() {
  return (
    <section id="how" className="relative z-10 scroll-mt-24 text-[#1F1E1B] hr-readable">
        ...
        <ol className="relative border-l-2 border-[#B9A06B]/35 ml-4 sm:ml-5 space-y-6 mb-7">
          {STEPS.map((s) => (
            <li key={s.n} className="relative pl-6 sm:pl-7">
              ...
              <div className="text-[15.5px] sm:text-[15px] font-bold text-[#1F1E1B] leading-[1.5]" style={MINCHO}>{s.t}</div>
              <p className="mt-1 text-[14px] text-[#45443E] leading-[1.9]">{s.d}</p>
            </li>
          ))}
        </ol>

        {/* 前提を1箇所に集約（探させない）。3枚のカードより、1枚の表のほうが短く読める。 */}
        <dl className="rounded-[1.1rem] bg-white border border-[#1F1E1B]/10 px-5 sm:px-6 py-5 mb-9 max-w-2xl">
          {TERMS.map(([t, d], i) => (
            <div
              key={t}
              className={`flex flex-col sm:flex-row sm:gap-4 ${i > 0 ? "mt-3 pt-3 border-t border-[#1F1E1B]/10" : ""}`}
            >
              <dt className="shrink-0 sm:w-[7em] text-[13.5px] font-bold text-[#1F1E1B]" style={MINCHO}>{t}</dt>
              <dd className="mt-0.5 sm:mt-0 text-[13.5px] text-[#45443E] leading-[1.85]">{d}</dd>
            </div>
          ))}
        </dl>
      </div>
