...
export default function PlanPricing() {
  return (
    <section id="pricing" className="relative z-10 scroll-mt-24 text-[#1F1E1B] hr-readable">
      <div className="max-w-[880px] mx-auto px-5 sm:px-8 py-14 sm:py-20">
        <p className="hr-eyebrow mb-3.5">何をするのか</p>
        <h2
              ...
              {STEPS.map((x, i) => (
                <li key={x.n} className="flex gap-3.5 pb-4 last:pb-0">
                  <div className="flex flex-col items-center shrink-0">
                    ...
                    {i < STEPS.length - 1 && (
                      <span aria-hidden className="w-px flex-1 bg-[#DAD6CD] mt-1.5" />
                    )}
                  </div>
                  <div className="pt-0.5">
                    <p className="text-[15px] font-bold text-[#1F1E1B] leading-[1.6]">{x.t}</p>
                    <p className="mt-0.5 text-[13.5px] text-[#45443E] leading-[1.8]">{x.d}</p>
                  </div>
                </li>
              ))}
            </ol>
            ...
            <ul className="mt-5 grid sm:grid-cols-2 gap-x-7 gap-y-4">
              {DELIVERABLES.map((d, i) => (
                <li key={d.t} className="flex gap-3">
                  ...
                  <div>
                    <p className="text-[14.5px] font-bold text-[#1F1E1B] leading-[1.55]">{d.t}</p>
                    <p className="mt-0.5 text-[13.5px] text-[#45443E] leading-[1.85]">{d.d}</p>
                  </div>
                </li>
              ))}
            </ul>
            <p className="mt-5 pt-4 border-t border-[#1F1E1B]/10 text-[13.5px] text-[#45443E] leading-[1.9]">
              加えて、実施日から<span className="text-[#1F1E1B] font-semibold">{SUPPORT.days}日間</span>
              は{SUPPORT.channel}で質問していただけます（{SUPPORT.note}）。
              ただし、<span className="text-[#1F1E1B] font-semibold">お渡ししたものを必ず再現できるようになること</span>
        ...
        <div className="mt-6 rounded-[1.4rem] bg-white border border-[#1F1E1B]/10 px-6 sm:px-8 py-6">
          ...
          <dl className="mt-5 pt-4 border-t border-[#1F1E1B]/10 text-[13.5px] leading-[1.9]">
            ...
            <div className="mt-2 flex gap-3">
              <dt className="shrink-0 w-[5.5em] font-bold text-[#5E6A70]">含みません</dt>
              <dd className="text-[#45443E]">{OUT_OF_POCKET.join("／")}（いずれも実費）</dd>
            </div>
          </dl>
          <p className="mt-4 text-[13px] text-[#5E6A70] leading-[1.9]">
