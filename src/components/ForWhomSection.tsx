      ...
      <div className="max-w-[880px] mx-auto px-5 sm:px-8 py-14 sm:py-20">
        ...
        <div className="mt-8 grid sm:grid-cols-2 gap-3 sm:gap-4">
          <div className="rounded-[1.4rem] bg-[#16241A] text-[#EDF1E8] p-5 sm:p-6">
            <p className="text-[15px] font-bold mb-3" style={MINCHO}>
              こういう方へ
            </p>
            <ul className="space-y-2.5">
              {FOR.map((x) => (
                <li key={x} className="flex gap-2.5 text-[14px] leading-[1.85]">
                  <span aria-hidden className="text-[#9ec4a3] shrink-0">✓</span>
                  <span>{x}</span>
                </li>
              ))}
            </ul>
            <p className="mt-4 pt-3 border-t border-white/12 text-[12.5px] text-[#9ec4a3] leading-[1.8]">
          ...
          <div className="rounded-[1.4rem] bg-white border border-[#1f2a1d]/10 p-5 sm:p-6">
            ...
            <ul className="space-y-2.5">
              {NOT_FOR.map((x) => (
                <li key={x} className="flex gap-2.5 text-[14px] text-[#5c6b58] leading-[1.85]">
                  <span aria-hidden className="text-[#9aa79a] shrink-0">—</span>
                  <span>{x}</span>
                </li>
              ))}
            </ul>
            <p className="mt-4 pt-3 border-t border-[#1f2a1d]/10 text-[12.5px] text-[#6b7a66] leading-[1.8]">
              当てはまる場合は、
