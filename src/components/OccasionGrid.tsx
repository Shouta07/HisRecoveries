...
export default function OccasionGrid({
  ...
  return (
    <section id="occasions" className="relative z-10 scroll-mt-24 text-[#1f2a1d]">
      <div className="max-w-[1100px] mx-auto px-5 sm:px-8 pt-12 sm:pt-16 pb-2">
        <div className="max-w-2xl mb-7">
          ...
          <h2 className="text-[1.5rem] sm:text-[1.9rem] leading-[1.35]" style={{ ...MINCHO, fontWeight: 800 }}>
            あなたは、<span className="text-[#3d5638]">何を叶えたい</span>ですか？
          </h2>
          <p className="mt-3 text-[13px] sm:text-[14px] text-[#4b5b47] leading-[1.9]">
            必要な変化は、悩みではなく<span className="font-semibold text-[#1f2a1d]">叶えたいこと</span>から決まります。
            近いものをひとつ選んでください。そこから逆算して、
            <span className="font-semibold text-[#1f2a1d]">あなたに必要な変化の順番</span>を設計します。
          </p>
        </div>
          ...
          {occasions.map((o) => {
            ...
            return (
              <button
                ...
                aria-pressed={on}
                className={`group flex flex-col text-left rounded-[1.4rem] border p-5 sm:p-6 transition-colors ${
                  on
              ...
              >
                ...
                <p className={`mt-2.5 text-[12.5px] font-semibold leading-[1.8] ${on ? "text-[#D7DED2]" : "text-[#3d5638]"}`}>
                  {o.job}
                </p>
                <p className={`mt-1.5 text-[11.5px] leading-[1.75] ${on ? "text-[#9FB0A0]" : "text-[#6b7a66]"}`}>
                  {o.lead}
                </p>
                  ...
                  {o.examples.map((e) => (
                    <li
                      ...
                      className={`rounded-full px-2.5 py-1 text-[10.5px] leading-[1.5] ${
                        on ? "bg-white/[0.08] text-[#D7DED2]" : "bg-[#eef3ea] text-[#3d5638]"
                      }`}
                    >
                      {e}
                    </li>
