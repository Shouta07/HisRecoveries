...
type Bloods = Record<string, string>;
...
function overallOfBloods(v: Bloods): number {
...
// ── Accord フィード ───────────────────────────────────────────────
// これは Accord がこの会員向けに配信するデータの型（を、デモ用にモックしたもの）。
// 相談・検査・診断・施術・メンテといった来訪、担当の一言、クリニックの検査値まで、
// すべて Accord 側で発生し、会員ビューはそれを表示するだけ。
type Kind = "相談" | "検査" | "診断" | "施術" | "メンテ";
type Visit = {
  id: string;
  kind: Kind;
  place: "オンライン" | "通院" | "来店";
...
type Feed = { goal: string; updatedAt: string; coordinator: string; visits: Visit[] };
...
const FEED_EMPTY: Feed = { goal: "", updatedAt: "—", coordinator: "", visits: [] };
