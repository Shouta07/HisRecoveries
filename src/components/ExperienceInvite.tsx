...
export default function ExperienceInvite({ context }: { context?: string }) {
  return (
    <aside className="my-12 rounded-[1.6rem] bg-[#1E2A38] text-[#F3F0EA] p-7 sm:p-9 overflow-hidden relative">
      ...
      <div className="relative">
        ...
        <h3 className="text-[1.3rem] sm:text-[1.55rem] font-bold text-[#F3F0EA] leading-[1.45] mb-3" style={MINCHO}>
          ...
          ひとりで抱えず、<span className="text-[#C28863]">30日で整える。</span>
        </h3>
        <p className="text-[14.5px] text-[#C6CAD0] leading-[1.95] max-w-[34rem] mb-4">
        ...
        <div className="flex flex-wrap gap-2 mb-6">
          {STEPS.map((s, i) => (
            <span key={s} className="inline-flex items-center gap-1.5 rounded-full bg-white/[0.07] border border-white/10 px-3 py-1 text-[12.5px] text-[#D3D7DC]">
              <span className="text-[#C28863] font-mono text-[10px]">{String(i + 1).padStart(2, "0")}</span>
              {s}
            </span>
          ))}
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          <BookingCTA className="bg-[#F3F0EA] hover:bg-white text-[#1E2A38] text-sm font-semibold px-7 py-3.5 rounded-full transition-colors">
            無料で相談する
          </BookingCTA>
        ...
        <p className="mt-4 text-[12px] text-[#8E979E] leading-[1.8]">
...
export function InlineConsult() {
  return (
    <div className="my-9 flex flex-col sm:flex-row sm:items-center gap-3 rounded-[1.1rem] border border-[#97613F]/20 bg-[#EDE9E0]/70 px-5 py-4">
      <p className="text-[14px] text-[#45443E] leading-[1.75] flex-1">
        読むだけでも大丈夫。でも「自分に合う形」が知りたくなったら、いつでも無料で相談できます。
      </p>
      <ConsultLink className="shrink-0 inline-flex items-center gap-1 text-[14px] font-semibold text-[#97613F] hover:opacity-70 transition-opacity whitespace-nowrap">
