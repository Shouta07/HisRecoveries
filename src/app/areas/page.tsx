...
function badgeStyle(b: Card["badge"]) {
  if (b === "取材") return "bg-[#97613F] text-white";
  if (b === "ガイド") return "bg-[#EDE9E0] text-[#97613F]";
  if (b === "選び方") return "bg-[#1E2A38] text-[#F3F0EA]";
  return "bg-[#EDE9E0] text-[#5E6A70]";
}
...
function ArticleCard({ c, carousel = false, starter = false }: { c: Card; carousel?: boolean; starter?: boolean }) {
  return (
    <Link
      ...
      className={`group flex flex-col rounded-[1.3rem] bg-white border border-[#1F1E1B]/10 p-5 hover:border-[#97613F]/40 hover:shadow-[0_18px_38px_-24px_rgba(20,32,26,0.5)] hover:-translate-y-0.5 transition-all${carousel ? " snap-start shrink-0 w-[78%] sm:w-auto" : ""}`}
    >
      <div className="flex items-center gap-2 mb-2">
        <span className={`inline-flex rounded-full px-2.5 py-0.5 text-[10px] font-bold ${badgeStyle(c.badge)}`}>{c.badge}</span>
        {starter && (
          <span className="ml-auto inline-flex items-center gap-1 rounded-full bg-[#1E2A38] text-[#F3F0EA] px-2 py-0.5 text-[11px] font-bold tracking-[0.04em]">
            <span aria-hidden>★</span>はじめの1本
          </span>
        )}
      </div>
      <h3 className="text-[15px] font-bold text-[#1F1E1B] leading-[1.55] group-hover:text-[#97613F] transition-colors" style={HEAD}>{c.title}</h3>
      <p className="mt-2 text-[14px] text-[#45443E] leading-[1.85] line-clamp-2 sm:line-clamp-3 flex-1">{c.excerpt}</p>
      <span className="mt-3 text-[13.5px] font-semibold text-[#97613F] inline-flex items-center gap-1 group-hover:gap-1.5 transition-all">読む <span aria-hidden>→</span></span>
    </Link>
  );
}
  ...
  const desireCards: Card[] = activeDesire
    ? clustersByDesire(activeDesire).map((a) => {
        ...
        return {
          ...
          accent: cx?.accent ?? "#97613F",
          accentSoft: cx?.accentSoft ?? "#EDE9E0",
          category: cx?.ja ?? "",
        };
      })
    : [];
  ...
  return (
    <div className="bg-[#F3F0EA] text-[#1F1E1B]">
      ...
      <div className="mx-auto max-w-[1080px] px-5 sm:px-8 pt-12 sm:pt-16 pb-24">
        {/* パンくず */}
        <nav aria-label="パンくず" className="text-[13.5px] text-[#5E6A70] mb-7">
          <Link href="/" className="hover:text-[#1F1E1B]">ホーム</Link>
          <span className="mx-1.5">/</span>
          <span className="text-[#1F1E1B]">記事</span>
        </nav>
        ...
        <header className="mb-10 max-w-2xl">
          <div className="flex items-center gap-3 mb-4">
            ...
            <span className="font-mono text-[12.5px] tracking-[0.18em] uppercase text-[#97613F] font-medium">His Recoveries Library</span>
          </div>
          <h1 className="text-[1.9rem] sm:text-[2.5rem] leading-[1.26]" style={HEAD}>
            <W>男の「<span className="text-[#97613F]">よくなる</span>」を、</W><W>まとめています。</W>
          </h1>
          <p className="mt-4 text-[15px] sm:text-[15px] text-[#45443E] leading-[1.9]">
        ...
        <div className="mb-12">
          ...
          {activeDesire && (
            <div className="mt-6">
              <div className="flex items-center gap-3 mb-4">
                <h3 className="text-[1.05rem] font-bold text-[#1F1E1B]" style={HEAD}>
                  「{DESIRES[activeDesire].label}」に効く記事
                  <span className="ml-2 text-[13.5px] font-normal text-[#5E6A70]">
                    {desireCards.length}本
                  </span>
                </h3>
                <Link
                  href="/areas#mokuteki"
                  className="ml-auto text-[13.5px] font-semibold text-[#97613F] hover:opacity-70 transition-opacity whitespace-nowrap"
                >
                  × 絞り込みを解除
                </Link>
              </div>
              {desireCards.length > 0 ? (
                ...
                <p className="text-[14.5px] text-[#45443E]">この目的の記事は準備中です。</p>
              )}
            </div>
          )}
        </div>
        ...
        <div className="sticky top-[62px] sm:top-[72px] z-30 -mx-5 sm:mx-0 mb-11 border-b border-[#1F1E1B]/10 bg-[#F3F0EA]/92 backdrop-blur-sm">
          <div className="flex sm:flex-wrap gap-2 overflow-x-auto sm:overflow-visible px-5 sm:px-0 py-3.5 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            {categories.map((c) => (
              <a key={c.id} href={`#cat-${c.id}`} className="shrink-0 inline-flex items-center gap-1.5 rounded-full border border-[#1F1E1B]/12 bg-white px-3.5 py-1.5 text-[14px] font-medium text-[#45443E] hover:border-[#97613F]/50 hover:text-[#1F1E1B] transition-colors">
                {c.ja}
              </a>
            ))}
            <a href="/areas/confidence" className="shrink-0 inline-flex items-center gap-1.5 rounded-full border border-[#1F1E1B]/12 bg-white px-3.5 py-1.5 text-[14px] font-medium text-[#45443E] hover:border-[#97613F]/50 hover:text-[#1F1E1B] transition-colors">
              自信・パートナーシップ
            </a>
            <a href="#cat-interview" className="shrink-0 inline-flex items-center gap-1.5 rounded-full border border-[#97613F]/25 bg-white px-3.5 py-1.5 text-[14px] font-medium text-[#97613F] hover:border-[#97613F] transition-colors">
              インタビュー
            </a>
          </div>
        </div>
        ...
        <div className="space-y-12">
          {categories.map((c) => (
            <section key={c.id} id={`cat-${c.id}`} className="scroll-mt-[128px]">
              <div className="flex items-center gap-2.5 mb-5">
                ...
                <h2 className="text-[1.25rem] font-bold text-[#1F1E1B]" style={HEAD}>{c.ja}</h2>
                <Link href={`/areas/${c.id}`} className="ml-auto text-[13.5px] font-semibold text-[#97613F] hover:opacity-70 transition-opacity whitespace-nowrap">
                  まとめて見る →
                </Link>
              </div>
              ...
              {c.cards.length > 1 && (
                <p className="sm:hidden mt-2.5 text-[12px] text-[#5E6A70]">← 横にスクロール（{c.cards.length}件）</p>
              )}

              {/* 最大カテゴリの直後に、静かな相談バンドを1つだけ（中間CVRポイント） */}
              {c.id === "impression" && (
                <div className="mt-8 rounded-[1.3rem] bg-[#1E2A38] text-[#F3F0EA] p-6 sm:p-7 flex flex-col sm:flex-row sm:items-center gap-4">
                  <div className="flex-1">
                    <p className="text-[15.5px] font-bold leading-[1.6]"><W>調べ続けるより、</W><W>10分の相談が早いこともあります。</W></p>
                    <p className="mt-1 text-[14px] text-[#C6CAD0] leading-[1.8]"><W>何から始めるか、いくらかかるか。</W><W>無料・完全守秘で。</W></p>
                  </div>
                  <ConsultLink className="shrink-0 inline-flex items-center justify-center gap-2 rounded-full bg-[#F3F0EA] hover:bg-white text-[#1E2A38] text-[14.5px] font-bold px-6 py-3 transition-colors">
          ...
          <section id="cat-interview" className="scroll-mt-[128px]">
            <div className="flex items-center gap-3 mb-2">
              <span aria-hidden className="grid place-items-center w-9 h-9 rounded-full shrink-0 bg-[#97613F]/10">
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#97613F" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round">
                  <path d="M21 11.5a8.5 8.5 0 0 1-11.7 7.9L4 21l1.6-5.3A8.5 8.5 0 1 1 21 11.5z" />
                </svg>
              </span>
              <h2 className="text-[1.3rem] font-bold text-[#1F1E1B]" style={HEAD}>インタビュー</h2>
            </div>
            <p className="text-[14px] text-[#5E6A70] leading-[1.9] mb-5">
            ...
            {interviews.length > 0 ? (
              ...
              <div className="rounded-[1.3rem] border border-dashed border-[#1F1E1B]/15 bg-white/50 p-8 text-center">
        ...
        <div className="mt-16 rounded-[1.3rem] border border-[#1F1E1B]/10 bg-white p-5 sm:p-6 flex flex-col sm:flex-row sm:items-center gap-4">
          <p className="text-[14.5px] text-[#45443E] leading-[1.85] flex-1">
            <W>読むだけでも大丈夫。</W><W>聞きたくなったら、無料で相談できます（完全守秘）。</W>
          </p>
          <div className="flex flex-wrap items-center gap-2.5 shrink-0">
            <ConsultLink className="inline-flex items-center gap-2 rounded-full bg-[#1E2A38] hover:bg-[#2A3849] text-white text-[14.5px] font-bold px-6 py-3 transition-colors">
              無料で相談する（無料） <span aria-hidden>→</span>
            </ConsultLink>
            <Link href="/plan" className="inline-flex items-center gap-2 rounded-full border border-[#1F1E1B]/20 hover:border-[#1F1E1B] text-[#1F1E1B] text-[14.5px] font-semibold px-6 py-3 transition-colors">
              プランを見る
            </Link>
          </div>
        </div>

        <p className="mt-8 text-[12.5px] text-[#5E6A70] leading-[1.9]">
